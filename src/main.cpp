...
#if defined(NDEBUG)
# error "Knifemoney cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xebe4edc6f2dcdffbb4bc2708d4efd8ce5178e464919966a349d96380bd9006c2");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Knifemoney: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Knifemoney Signed Message:\n";
...
{
    // Knifemoney: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Knifemoney: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Knifemoney: 3.5 days
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Knifemoney: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Knifemoney: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Lorem ipsum dolor sit amet, consectetur adipiscing elit.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1440564994;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1721480;
        ...
        assert(block.hashMerkleRoot == uint256("0xb2a4609b9558285619dd8fd24fc23541fb0619197725dea58c8ef1103f26cb70"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Knifemoney: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("knifemoneyMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("knifemoneyMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static knifemoneyMiner(CWallet *pwallet)
{
    printf("knifemoneyMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("knifemoney-miner");
    ...
    try { loop {
        ...
        printf("Running knifemoneyMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("knifemoneyMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&knifemoneyMiner, pwallet));
}
